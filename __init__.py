...
def is_valid_password(password):
    return (
        len(password) <= 10 and # Password maksimal 10 karakter
...
def sign_up():
    ...
    if cek_ulang.lower() == "ya":
        while True:
            username = input("Masukkan username (maksimal 12 karakter dan harus memuat angka): ")
            if (
                len(username) > 12 or
                ...
                break
        while True:
            password = getpass.getpass("Masukkan password (maksimal 10 karakter, huruf kapital, angka, dan simbol unik): ")
